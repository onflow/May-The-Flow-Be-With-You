...
access(all) contract EvolvingCreatures: NonFungibleToken {
    ...
    init() {
        ...
        // Set up the main account that deploys the contract with an empty collection
        // This is often done in a separate transaction, but can be done here if the deployer
        // should also be able to mint or hold NFTs from the get-go.
        // However, standard practice is to have users call a "setup" transaction.
        // So, leaving this out of init() for now.
