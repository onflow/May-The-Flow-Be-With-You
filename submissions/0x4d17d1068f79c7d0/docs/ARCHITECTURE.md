...
export class MemoryGameEngine {
  ...
  static checkAchievements(userStats: UserStats, newScore: Score): Achievement[]
}
...
export class CulturalEngine {
  ...
  static adaptGameForCulture(baseGame: GameConfig, culture: Culture): GameConfig
...
export class OffChainAdapter implements GameAdapter {
  ...
  async saveProgress(userId: string, progress: GameProgress): Promise<void> {
    ...
    await this.supabase.from('progress').upsert({ user_id: userId, ...progress })
    this.localStorage.setItem(`progress_${userId}`, JSON.stringify(progress))
  }
  
  supportsFeature(feature: GameFeature): boolean {
    return !['nft_achievements', 'tournaments', 'verifiable_randomness'].includes(feature)
  }
}
...
export class OnChainAdapter implements GameAdapter {
  ...
  async unlockAchievement(userId: string, achievement: Achievement): Promise<void> {
    ...
    const nftId = await this.flowService.mintAchievementNFT(userId, achievement)
...
export class GameService {
  ...
  async playGame(config: GameConfig): Promise<GameSession> {
    ...
    // 2. Adapt for culture
    const culturalSequence = CulturalEngine.adaptGameForCulture(sequence, config.culture)
    
    // 3. Return session (no side effects yet)
    return new GameSession(culturalSequence, config)
  }
  
  async submitResult(session: GameSession, userAnswer: any[]): Promise<GameResult> {
    // 1. Validate using pure engine
    const result = MemoryGameEngine.validateAnswer(session.sequence, userAnswer)
    ...
    const newAchievements = MemoryGameEngine.checkAchievements(userStats, score)
    
    // 3. Persist using adapter (mode-specific)
    await this.adapter.saveProgress(session.userId, { ...userStats, lastScore: score })
  ...
  getAvailableFeatures(): GameFeature[] {
    return ALL_FEATURES.filter(feature => this.adapter.supportsFeature(feature))
  }
}
```
...
export function ModeSelector() {
  const { user } = useAuth()
  const [currentMode, setCurrentMode] = useState<'offchain' | 'onchain'>('offchain')
  
  const gameService = useMemo(() => {
    const adapter = currentMode === 'onchain' 
  ...
  return (
    <div className="mode-selector">
      <div className="mode-toggle">
        ...
        >
          🏆 Competitive Mode
          <span className="subtitle">NFT achievements, global leaderboards</span>
        </button>
      </div>
